...
```{r, echo=TRUE, message=FALSE, warning=FALSE}
allpost <- as.data.table(allpost)

allpost_tokenized <- allpost %>%
  .[, .(post, doc_id)] %>%
  .[, word := purrr::map(post, function(x) segment(x, jieba))] %>%
  unnest(word) %>%
  .[!stopWords, on = "word"] %>%
...
topic_classification <- tidy(lda_model, matrix = "gamma") %>%
  as.data.table() %>%
  .[order(gamma), tail(.SD, 1), document] %>%
  .[gamma > 0.2, .(doc_id = document, topic, gamma)]

post_with_topic <- topic_classification_1 %>%
  .[, gamma := NULL] %>%
  allpost[., on = "doc_id"] %>%
  .[, topic_name := case_when(topic == 1 ~ "3C和遊戲", topic == 2 ~ "偏激言論討論",
...
post_with_topic_plot <- post_with_topic %>%
  .[, .(topic_name, timestamp = str_replace(timestamp, "  ", " "))] %>%
  separate(timestamp, c("day", "month", "date"), sep = " ") %>%
  .[, .(topic_name, month, date = as.numeric(date))] %>%
  .[, week := case_when(month == "Feb" & date <= 7 ~ "Feb w1",
                        month == "Feb" & date <= 14 & date > 7 ~ "Feb w2",
                        month == "Feb" & date <= 21 & date > 14 ~ "Feb w3",
                        month == "Feb" & date <= 28 & date > 21 ~ "Feb w4",
                        month == "Mar" & date <= 7 ~ "Mar w1",
                        month == "Mar" & date <= 14 & date > 7 ~ "Mar w2",
                        month == "Mar" & date <= 21 & date > 14 ~ "Mar w3",
                        TRUE ~ "Mar w4")] %>%
  .[, .N, .(topic_name, week)]
```
...
igraph_item <- graph.data.frame(edge_list_1, directed = FALSE)
...
```{r, echo=TRUE, message=FALSE, warning=FALSE}
uniuser_number <- as.data.table(bipartite_matrix) %>%
  .[, sum := rowSums(.SD)] %>%
  .[sum <= 2] %>%
  .[, lapply(.SD, sum)] %>%
  .[, sum := NULL]
